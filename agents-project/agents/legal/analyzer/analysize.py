"""法律文档风险分析 — RAG + LLM 分析主流程

接收 file_id，通过 RAG 分页读取文档切片并检索相关法条，
按输入预算分批提交 LLM 分析风险，最后 consolidation 去重归并后输出。
"""
...
_scan_batch_model = create_deepseek_model(temperature=0.1).with_structured_output(BatchRiskPage)
_consolidate_model = create_deepseek_model(temperature=0.1).with_structured_output(ConsolidatedReport)
...
def _build_legal_context(laws: list[dict]) -> str:
    """把该切片的检索法条拼成提示词里的法律依据块。"""
    if not laws:
        return "（未检索到直接相关的法律条文，请基于法律常识判断。）"
    return "\n".join(
        f"[{r['metadata'].get('heading', '法条')}] {r['content']}"
        for r in laws
    )
...
def _group_into_batches(chunks: list[dict], law_results: list[list[dict]]) -> list[list[tuple[dict, str]]]:
    ...
    for i, chunk in enumerate(chunks):
        ...
        legal_context = _build_legal_context(laws)
...
def analyze_file(file_id: str) -> dict:
    ...
    两阶段：
    1. 分页取回切片（内存上界 PAGE_SIZE），按输入预算分批并发扫描
    2. consolidation 去重归并 → 输出最终报告
    """
    from agents.RAG import get_chunk_num, get_chunk, retrieve_chunk

    total = get_chunk_num(file_id)
    if total == 0:
        return {"error": "该文件已过期或无切片数据，请重新上传文件后再试。"}

    raw_risks: list[dict] = []

    # ── 阶段一：分页取回（内存上界 PAGE_SIZE）+ 分批并发扫描 ──
    ...
    with ThreadPoolExecutor(max_workers=SCAN_WORKERS) as pool:
        for page in range(1, pages + 1):
            ...
            try:
                law_results = retrieve_chunk(chunks)
            except Exception:
                law_results = []
    ...
    # 少量风险时跳过 LLM 归并，用代码直接汇总，省一次 LLM 调用（约 20~25s）
    if len(raw_risks) <= CONSOLIDATE_MIN_RISKS:
        order = {"high": 3, "medium": 2, "low": 1}
        risk_score = max(raw_risks, key=lambda r: order.get(r["risk_level"], 0))["risk_level"]
        return {
            "file_id": file_id,
            "total_chunks": total,
            "risk_score": risk_score,
            "summary": f"共发现 {len(raw_risks)} 处风险点，整体风险等级为{risk_score}。",
            "total_risks": len(raw_risks),
            "risks": [{**r, "affected_chunks": [r["chunk_index"]]} for r in raw_risks],
        }

    # ── 阶段二：consolidation 去重归并 ──
    try:
        consolidated = _consolidate_model.invoke([
            CONSOLIDATE_PROMPT.format(
                raw_risks_json=json.dumps(raw_risks, ensure_ascii=False),
                total_chunks=total,
            )
        ])
