...
from agents.RAG import retrieve_text, get_chunk, get_chunk_num
...
4. 权利义务条款应当对等、公平
5. 必须包含管辖条款、送达条款
6. 引用法律时，必须基于"相关法律依据"中提供的条文，注明具体法律名称和条款；依据中未提供的，不得虚构具体法条编号，可用"根据相关法律规定"表述
...
5. 输出完整文书，不得只输出修改的部分
6. 引用法律时，必须基于"相关法律依据"中提供的条文，注明具体法律名称和条款；依据中未提供的，不得虚构具体法条编号
7. 禁止输出任何解释性文字，只输出文书正文本身
8. 文书内容必须使用中文"""

_model = create_deepseek_model(temperature=0.3)
_condense_model = create_deepseek_model(temperature=0.1)
...
def draft_document(
    ...
    # RAG 检索相关法律法规作为起草依据（失败时降级，不阻塞起草）
    try:
        law_results = retrieve_text(f"{doc_type} {requirement}", top_k=6)
        legal_basis = (
            "\n\n".join(
            ...
            if law_results
            else "（未检索到直接相关法条，可基于法律常识起草）"
        )
    except Exception:
        legal_basis = "（法律检索服务不可用，可基于法律常识起草）"
    ...
    if original_content:
        ...
        response = _model.invoke([
            ...
            HumanMessage(content=f"请基于这份{doc_type}按要求起草/修订：{rev_instructions}"),
        ])
    else:
        # 初次生成
        response = _model.invoke([
            ...
            HumanMessage(content=f"请起草一份{doc_type}，需求：{requirement}"),
        ])

    title, sections = _parse_llm_output(response.content)
